...
def _validate_cached_context_length(model: str, base_url: str, cached: int, is_bedrock_context: bool, *, api_key: str = "") -> Optional[int]:
    ...
    if _infer_provider_from_url(base_url) == "nous":
        ...
        return None
    # ssoni (OmniRoute): /v1/models is authoritative (rich per-model context_length). Bypass
    # (don't drop) the cache so the step-5d2 probe reconciles entries seeded from the hardcoded
    # DEFAULT_CONTEXT_LENGTHS table without touching disk when the endpoint is unreachable. See #22401.
    if _infer_provider_from_url(base_url) == "ssoni":
        logger.debug("Bypassing persistent cache for %s@%s (ssoni /v1/models authoritative)", model, base_url)
        return None
...
def _resolve_provider_aware_context_length(model: str, base_url: str, api_key: str, provider: str, effective_provider: str) -> Optional[int]:
    ...
    if effective_provider in {"gmi", "commandcode", "commandcode-anthropic"} and base_url:
        ...
        if ctx is not None:
            return ctx
    # 5d2. ssoni (OmniRoute) — exposes authoritative context_length via its OpenAI-compatible
    # /v1/models endpoint (rich per-model metadata). ssoni is a known provider (auto-registered in
    # _URL_TO_PROVIDER), so the generic custom-endpoint probe at step 2 is skipped for it — and
    # models.dev has no ssoni data — so without this branch the resolver falls through to the
    # hardcoded DEFAULT_CONTEXT_LENGTHS table, which can disagree with the provider's actual cap
    # (e.g. opencode-go/deepseek-v4-pro resolves to 1,000,000 via the deepseek-v4-pro substring, but
    # ssoni serves it at 200,000). Probe the live endpoint and persist the result. See #22401.
