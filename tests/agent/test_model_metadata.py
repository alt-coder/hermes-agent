...
import agent.model_metadata as _model_metadata_module
...
class TestSsoniProviderContextResolution:
    ...
    def setup_method(self, _method):
        # Mirror production: in a real install, ssoni is a registered provider
        # profile whose hostname is auto-extended into _URL_TO_PROVIDER, so
        # _infer_provider_from_url() returns "ssoni", step 2 (generic
        # custom-endpoint probe) is skipped, and the dedicated ssoni branches
        # (cache bypass + step 5d2) fire. pytest isolates HERMES_HOME to a
        # tempdir, so the ssoni plugin is not loaded and the host would be
        # missing — register it explicitly to exercise the same code paths.
        _model_metadata_module._URL_TO_PROVIDER["api.ssoni.net"] = "ssoni"

    def teardown_method(self, _method):
        _model_metadata_module._URL_TO_PROVIDER.pop("api.ssoni.net", None)
